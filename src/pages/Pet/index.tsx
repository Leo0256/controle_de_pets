...
const Pet:FC<{ navigation: any }> = (props) => {
  ...
  useEffect(()=>{
    async function list():Promise<void> {
      setLoading(true);
      const response = petList ? await petList() as {pets:PetType[], count:number} : undefined;
      if (response) {
        if( response.pets ){
          console.log(JSON.stringify(response.pets))
  ...
  const RenderItem:FC<{item:PetType}> = ({ item }) => {
    ...
    <View style={styles.item}>
      <TouchableOpacity style={styles.itemtext} onPress={()=> {
        ...
        <Text style={[styles.itemname,selected == item.idpet &&{fontWeight:'bold'}]}>zzz{item.name}</Text>
        { selected == item.idpet &&
    ...
    <View style={styles.container}>
      {
        ...
        <ScrollView style={styles.scroll}>
          <FlatList
            ...
            keyExtractor={({item}:any) => (item.idpet as number).toString()}
          />
        </ScrollView>
        :
